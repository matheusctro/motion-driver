...
*/ 
  ...
  const handleUpload = async () => {
    let upload;
    upload = await api.get('/motions');
    dispatch({ type: 'LOAD_MOTIONS', motions: upload.data })
  }

  const handleDownload = async () => {

    let motions2download = { "motions": [] }
    motions2download.motions = motions;

    await api.post(`/motions`, motions2download);
  }
